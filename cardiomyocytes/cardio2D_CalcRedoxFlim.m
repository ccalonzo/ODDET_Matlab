%% Load metadata
[~,~,RunList] = xlsread('20141016_cardio.xlsx');
InputHeader = RunList(1,:);
RunList = RunList(2:end,:);
dataGroup = '20141016_cardio-fccp';
...
for m = 1:listLength
    ...
    well = RunList{m,2};
    field = RunList{m,3};
    power755 = RunList{m,6};
    power860 = RunList{m,7};

    %% Load image files
    fname = [cellType,'-',num2str(well),num2str(field)];
...
%     Threshold(m) = 500;  %in photon counts per pixel
    ...
    FADMask = (Int860) > 10;%FADThreshold(m);
    NADHThreshold(m) = adaptiveThreshold(Int755);
    NADHMask = (Int755) > 40;%NADHThreshold(m);
%     CellMask = antiSmall(NADHMask & FADMask, 10);
    CellMask = antiSmall(FADMask | NADHMask,5);
